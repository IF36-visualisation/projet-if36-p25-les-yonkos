...
p5 <- ggplot(plot_data_valid_fights, aes(x = win_rate, y = career_duration_years)) +
    ...
    labs(
      ...
      caption = "Source: Données UFC agrégées"
    ) +
    theme_bw() +
...
- **Zone optimale** : taux de victoire entre 50-60%
- **Carrières courtes** : taux très faibles (<20%) ou très élevés (>90%)
- **Courbe en dôme** : la longévité maximale correspond à un succès modéré mais constant

Néanmoins en regardant les noms des combattants avec le meilleur ratio, on voit que la majorité combat encore et que leur carrière n'est pas encore finie. Ainsi la tendance est croissante (et c'est logique). Plus on combattant gagne, et plus il semble rester longtemps dans l'UFC.
...
```{r damage_frequency_plot}
  ...
  print(p1)
```
**Interprétation** : On voit ici :

- **Zone d'augmentation** (0-2.5 coups/minute) : la longévité s'améliore avec un engagement modéré
- **Plateau optimal** (2.5-4 coups/minute) : zone de performance équilibrée
- **Déclin** (>5 coups/minute) : diminution significative de la durée de carrière

Cette courbe en cloche suggère qu'un certain niveau d'engagement est bénéfique, mais qu'une exposition excessive aux dommages devient problématique et peut visiblement causer assez de dommages pour arrêter une carrière par accumulation. Cela pourrait aussi simplement être lié au fait que plus un combattant est nul et plus il reçoit de coups, ce qui vient compléter le graphique précédent.
...
p2 <- ggplot(plot_data_base, aes(x = fight_frequency_per_year, y = career_duration_years)) +
...
**Interprétation** : La fréquence des combats montre également une relation optimale.

- **Pic de longévité** : environ 1 combat par an
- **Déclin rapide** : au-delà de 3 combats annuels
- **Carrières très courtes** : plus de 5 combats par an

Ces données indiquent l'importance d'une récupération adéquate entre les combats pour maintenir une carrière durable.
...
**Interprétation** : L'analyse des KO révèle une tendance claire :

- **Carrières courtes** (≤0.9 ans) : ~0.24 KO par combat
- **Carrières moyennes** (0.9-3.4 ans) : ~0.20 KO par combat  
- **Carrières longues** (>3.4 ans) : ~0.15 KO par combat
...
**Interprétation** : La distribution par catégorie de poids montre des disparités notables :

- **Catégories favorables** : poids moyens et mi-lourds présentent les médianes de longévité les plus élevées
- **Catégories moins favorables** : poids mouches masculins montrent des carrières généralement plus courtes
- **Variabilité importante** : les catégories féminines présentent une grande dispersion

La moyenne est globalement la même partout ce qui confirme la non-corrélation indiquée par la matrice. Le poids n'influent pas sur la durée de carrière.
...
Cette deuxième visualisation permet de vérifier ou non l'hypothèse précédente en visualisant les 10 combattants avec le meilleur ratio victoires/défaites/no contests. On va prendre un échantillon avec les combattants ayant le plus de combats.
...
# Charger les données
df <- read.csv("data/complete_ufc_data.csv")

# Nettoyage et construction des stats
df_clean <- df %>%
...
all_stats <- full_join(wins, losses, by = c("winner" = "loser")) %>%
  full_join(no_contests, by = c("winner" = "fighter")) %>%
  rename(fighter = winner) %>%
...
# Sélection des 10 meilleurs ratios
top_ratios <- all_stats %>% slice_max(ratio, n = 10)

# Restructuration des données pour barres empilées
top_ratios_long <- top_ratios %>%
  select(fighter, wins, losses, no_contests, ratio) %>%
  pivot_longer(cols = c(wins, losses, no_contests),
               names_to = "result",
               values_to = "count")

top_ratios_long$result <- factor(top_ratios_long$result, levels = c("wins", "losses", "no_contests"))

# Graphique barres empilées
ggplot(top_ratios_long, aes(x = reorder(fighter, ratio), y = count, fill = result)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(
    title = "Top 10 des combattants par nombre total de combats",
  ...
  theme_minimal() +
  scale_fill_manual(
    values = c("wins" = "forestgreen", "losses" = "firebrick", "no_contests" = "gray50"),
    labels = c("wins" = "Victoires", "losses" = "Défaites", "no_contests" = "No contests")
  )
```

**Interprétation :** 
Le graphique révèle que Jon Jones domine ce classement avec environ 23 combats au total, suivi de près par Georges St-Pierre avec 22 combats. On distingue ensuite un groupe intermédiaire composé de Demetrious Johnson, Kamaru Usman et Amanda Nunes, qui comptabilisent chacun entre 16 et 18 combats.
...
Le graphique montre de façon remarquable que tous ces combattants d'élite partagent une caractéristique commune : une écrasante majorité de victoires (représentées en vert) par rapport aux défaites (en rouge), avec Jon Jones qui se distingue comme le seul à avoir des combats sans décision (no contests, en gris) dans son palmarès.

Au regard de ce graphique et de son palmarès exceptionnel, Jon Jones s'impose indéniablement comme le combattant le plus spectaculaire de l'UFC, ayant non seulement accumulé le plus grand nombre de combats au plus haut niveau, mais ayant également maintenu une domination quasi-absolue tout au long de sa carrière légendaire.
...
jones_fights <- df %>%
  filter(fighter1 == "Jon Jones" | fighter2 == "Jon Jones") %>%
  mutate(
    event_date = as.Date(event_date),
    is_fighter1 = fighter1 == "Jon Jones",
    sig_strikes_accuracy = if_else(is_fighter1, fighter1_sig_strikes_accuracy, fighter2_sig_strikes_accuracy),
    takedown_avg_per15m = if_else(is_fighter1, fighter1_takedown_avg_per15m, fighter2_takedown_avg_per15m),
    sig_strikes_defended = if_else(is_fighter1, fighter1_sig_strikes_defended, fighter2_sig_strikes_defended),
...
jones_long <- jones_fights %>%
  select(event_date, outcome_binary, sig_strikes_accuracy, takedown_avg_per15m, sig_strikes_defended) %>%
  pivot_longer(cols = -event_date, names_to = "metric", values_to = "value") %>%
  filter(!is.na(value))

ggplot(jones_long, aes(x = event_date, y = value, color = metric)) +
  geom_step(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(
    values = c(
      outcome_binary = "darkgreen",
      sig_strikes_accuracy = "blue",
      takedown_avg_per15m = "purple",
      sig_strikes_defended = "orange"
    ),
    labels = c(
      outcome_binary = "Victoire (1 = oui, 0 = non)",
      sig_strikes_accuracy = "Précision des frappes (%)",
      takedown_avg_per15m = "Takedowns moyens / 15min",
      sig_strikes_defended = "Défense des frappes (%)"
    )
  ...
  labs(
    title = "Évolution des performances de Jon Jones",
...
**Interprétation :** 
Le graphique démontre de manière frappante la domination de Jon Jones avec une ligne verte des victoires maintenue constamment à 1.0 (victoire) sur pratiquement toute sa carrière de 2010 à 2020. Cette constance est remarquable dans un sport aussi imprévisible que les arts martiaux mixtes. La seule interruption visible vers 2011 correspond à une chute brutale vers 0, représentant sa seule défaite officielle dans cette période.

Cette quasi-perfection statistique illustre une maîtrise technique et mentale exceptionnelle qui lui a permis de maintenir un niveau d'excellence sur une décennie complète de compétition au plus haut niveau.
...
```{r load_data}
df <- read.csv("C:/Users/moham/Downloads/complete_ufc_data (2) (1).csv")
...
# Analyse des différences stratégiques entre les gagnants et les perdants
...
# Mise en avant des tendances favorisant la victoire
```{r graph_7}
...
#  Y'a t-il un secteur à privilégier pour espérer gagner
```{r graph_11}
